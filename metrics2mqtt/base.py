...
class MQTTMetrics(object):
    ...
    def create_config_topics(self):
        for metric in self.metrics:
            ...
            self.client.publish(
                metric.topics["config"], json.dumps(config_topic), retain=True, qos=1
            )
...
def get_arguments():
    ...
    parser.add_argument(
        "--port", default=1883, type=int, help="Port of the MQTT broker (default: 1883)"
    )
    parser.add_argument(
        "--username",
        default=None,
    ...
    parser.add_argument(
        "--net",
        help="Publish network interface metrics. Specify the interface name and collection interval seperated by a comma",
        type=str,
        action="append",
        nargs="?",
        const="/",
